    ...
    function filterGrid(virno, containerno, indexno, type, agent, goodhead, fromdate, todate, status) {
    ...
    //GetPortChargesDetail        old for igm index container and type wise
    $.get('/APICalls/GetPortChargesRates?virno=' + virno + '&containerno=' + containerno + '&indexno=' + indexno + '&type=' + type + '&agent=' + agent + '&goodhead=' + goodhead
    + '&fromdate=' + fromdate + '&todate=' + todate + '&status=' + status, function (data) {
    ...
    var goodhead = $("#goodsheaddropdown").val();
    ...
    filterGrid(virno, containerno, indexno, type, agent, goodhead, fromdate, todate, status);
        }
    ...
    if (resultvalue == true) {
        $.post('/Import/Setup/AddPortChargesList', { data: rowObject }, function (data) {
            ...
            if (data.error == true) {
                ...
                filterGrid(virno, containerno, indexno, ContainerType, agent, goodhead, fromdate, todate, status);
    ...
    // work start for per box


    function showcargoDetaildescPerBox() {

        var res = [];
    containerIndexesGridPerBox(res);

    var type = $('#ContainerTypePerBox').val();
    ...
    if (type == null) {
            ...
            var virno = $("#virnolistPerBox").dxAutocomplete("instance").option("value");

    var indexno = $("#containerIndexlistPerBox").dxAutocomplete("instance").option("value");

    var containerno = $("#containerlistPerBox").dxAutocomplete("instance").option("value");

    filterGridPerBox(virno, containerno, indexno, type);
        }

         
    }

    function containerIndexesGridPerBox(res) {

        containersPerBox = res;

    console.log("containersPerBox", containersPerBox)

    var grid = $("#PortchargesGridPerBox").dxDataGrid(this.gridSettings).dxDataGrid('instance');


    var dataGrid = $("#PortchargesGridPerBox").dxDataGrid({
        dataSource: containersPerBox,
    keyExpr: "key",
    wordWrapEnabled: true,
    showBorders: true,
    showBorders: true,
    ...
    function filterGridPerBox(virno, containerno, indexno, type) {
    ...
    $.get('/APICalls/GetPortChargesDetailContainerWise?virno=' + virno + '&containerno=' + containerno + '&indexno=' + indexno + '&type=' + type, function (data) {
        ...
        containersPerBox = data;

    containerIndexesGridPerBox(containersPerBox)
            }
    ...
    function SaveInfoPerBox() {

        var resultvalue = true;
    var rowObject = $("#PortchargesGridPerBox").dxDataGrid("option", "dataSource");
    ...
    if (resultvalue == true) {
        $.post('/Import/Setup/AddPortChargesContainerWise', { data: rowObject }, function (data) {
            ...
            if (data.error == true) {
                ...
                var containerno = $("#containerlistPerBox").dxAutocomplete("instance").option("value");
                var ContainerType = $("#ContainerTypePerBox").val();
                ...
                filterGridPerBox(virno, containerno, indexno, ContainerType);
