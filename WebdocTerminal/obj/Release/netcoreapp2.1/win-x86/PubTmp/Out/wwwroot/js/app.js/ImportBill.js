    ...
    if (clearingAgentdropdownid) {
            ...
            var getletpassamount = "";

    if ($("input[name='type']:checked").val() == "CFS") {
        getletpassamount = '/APICalls/PartyInfo?partyid=' + clearingAgentdropdownid + '&igm=' + igm + '&index=' + $("#indexdb option:selected").text() + '&type=' + $("input[name='type']:checked").val();

            }
    if ($("input[name='type']:checked").val() == "CY") {
        getletpassamount = '/APICalls/PartyInfo?partyid=' + clearingAgentdropdownid + '&igm=' + igm + '&index=' + $("#containerIndexCYdb option:selected").val() + '&type=' + $("input[name='type']:checked").val();

            }
    $.get(getletpassamount , function (res) {
                if (res) {
        $('#ledgerBalanceAmount').val(res.balance);
    $('#ledgerPartyid').val(res.partyId);
    $('#ledgerPartyNature').val(res.nature);
                }
    else {
        resetLedgerinfo()
    }
    ...
    function resetLedgerinfo() {
        $('#ledgerBalanceAmount').val('');
    $('#ledgerPartyid').val('');
    $('#ledgerPartyNature').val('');
    }
    ...
    document.getElementById('InsturmentNodiv').style.display = "none"

         } else {
        $("#InsturmentNo").prop("readonly", false);
    document.getElementById('InsturmentNodiv').style.display = "block"
    ...
    function AddPaymentNatureList() {
        ...
        if (PaymentNatureList.filter(t => t.natureOfAmount == "CashRefund" && t.receivedAmount > 0).length) {
            return alert("due to Cash Refund  you can't add more");
        }
    else {
    ...
    if (res == "CashRefund") {
        ...
        console.log("(PaymentNatureList.find(t => t.CreditAllowed > 0) ", (PaymentNatureList.filter(t => t.creditAllowed > 0)))
                if (PaymentNatureList.filter(t => t.creditAllowed > 0).length) {
                    return alert("due to credit allow you can't use cash refound");
                }
    var receivedAmountres = $('#receivedAmount').val();
    ...
    KnockOffAmount: Math.round($('#KnockOffAmount').val()),
    ...
    let sum = 0;
    sum += x.receivedAmount;
            PaymentNatureList.filter(obj => {sum += obj.receivedAmount; });

            if (PaymentNatureList.filter(t => t.creditAllowed > 0).length && sum > $('#inoviceTotalAmount').val()) {
                return alert("due to credit allow you can't use excess value");
            }
    ...
    PaymentReceivedGrid();
            ...
            //start auto cash refund work 

            if (!PaymentNatureList.filter(t => t.creditAllowed > 0).length && !PaymentNatureList.filter(t => t.natureOfAmount == "CashRefund").length) {

                if (Number($('#excessAmount').val()) > 0 && Number($('#excessAmount').val()) <= 5000) {

        let cx = {

        aictBankName: "",
    bankId: "",
    receivedAmount: Number($('#excessAmount').val()),
    InsturmentNo: "",
    creditAllowed: "0",
    InoviceNo: "",
    KnockOffAmount: "",
    ...
    allowDeleting: function (e) { 
                    return e.row.data.natureOfAmount == "CreditAllowed" ? false : true;
                },
            },
    columns: [
    {
    ...
    caption: "Inovice No",
        ...
        dataField: "KnockOffAmount",
    caption: "Knock Off Amount",
    dataType: "number",
    format: "#,##0.##",
                },{
    ...
    {
        dataField: "knockOffInvoiceNo",
    caption: "Knock Off Invoice No",
                },
    ],


    onRowRemoving: function (e) {
                if (PaymentReceiveGriddata.filter(x => x.natureOfAmount == "CashRefund").length && e.data.natureOfAmount != "CashRefund") {

        e.cancel = "true";
    return alert("please  delete CashRefund then delete others");
    ...
    var totalpaymentreceivedamount = 0;
    var cashrefoundamount = 0;

    if (PaymentReceiveGriddata.length) {
        PaymentReceiveGriddata.forEach(c => {
            if (c.natureOfAmount != "CashRefund") {

                totalpaymentreceivedamount += Number(c.receivedAmount);
            }
            if (c.natureOfAmount == "CashRefund") {

                cashrefoundamount += Number(c.receivedAmount);
            }
        });
                    }
    ...
    var aftertotalpaymentreceivedamount = 0

    aftertotalpaymentreceivedamount = totalpaymentreceivedamount - inoviceTotalAmount;


                    //if (Number(aftertotalpaymentreceivedamount) <= Number(5000) && Number(aftertotalpaymentreceivedamount) > 0 && Number(inoviceTotalAmount) > 0) {

                    //    let x = {

                    //        aictBankName: "",
                    //        receivedAmount: (- aftertotalpaymentreceivedamount),
                    //        InsturmentNo: "",
                    //        creditAllowed: 0,
                    //        InoviceNo: "",
                    //        KnockOffAmount: "",
                    ...
                    //    $("#excessAmount").val(Number(totalpaymentreceivedamount) - Number(inoviceTotalAmount))

                    //}
                    //else
                    //if (Number(totalpaymentreceivedamount) > Number(inoviceTotalAmount) && Number(inoviceTotalAmount) > 0 && Number(aftertotalpaymentreceivedamount) > Number(5000)) {


                    if (Number(totalpaymentreceivedamount) > Number(inoviceTotalAmount) && Number(inoviceTotalAmount) > 0) {
        $("#excessAmount").val((Number(totalpaymentreceivedamount) - Number(inoviceTotalAmount)) - Number(cashrefoundamount))
    } else {
        $("#excessAmount").val(0)
    }
    ...
    totalbalanceInoviceAmount = ((inoviceTotalAmount - totalpaymentreceivedamount) + (Number($("#excessAmount").val())))
    ...
    customizeText(data) {
        ...
        let totalamount = 0;
    let totalamountRefound = 0;

    let totalknockOffAmountg = 0;
    console.log("PaymentNatureList", PaymentNatureList)

                            
                            PaymentNatureList.forEach(c => {
                                          
                                    if (c.natureOfAmount != "CashRefund") {

        totalamount += Number(c.receivedAmount);
                                    }
    if (c.natureOfAmount == "CashRefund") {

        totalamountRefound += Number(c.receivedAmount);
                                    }

    if (c.natureOfAmount == "KnockOff") {

        totalknockOffAmountg += Number(c.receivedAmount);
                                    }
                                });

    $("#knockOffAmountg").val(totalknockOffAmountg);
    $("#paymentReceivedh").val(totalamount);

    return `Total Collection Received: ${totalamount} And Total Amount Refound: ${totalamountRefound}`;
                        },
                    }]
            },
    ...
    //setpaymentdetail(PaymentReceiveGriddata);
    ...
    aftertotalpaymentreceivedamount = totalpaymentreceivedamount - inoviceTotalAmount;
        ...
        //else if (Number(aftertotalpaymentreceivedamount) > Number(5000) && Number(aftertotalpaymentreceivedamount) > 0 && Number(inoviceTotalAmount) > 0) {

        //    PaymentNatureList = PaymentReceiveGriddata.filter(function (el) { return el.natureOfAmount != "CashRefund"; });

        //    console.log("PaymentNatureListtest", PaymentNatureList);
        //    resetOnlineReceiptCounterPayment();
        //    PaymentReceivedGrid();
    ...
    function setpaymentdetail(PaymentReceiveGriddata) {

        //var totalreceivedamount = 0;
        var restoftotalreceivedcashrefund = 0;
    var totalreceivedcashrefund = 0;
    var totalreceivedcreditallow = 0;
    var inoviceTotalAmount = 0;
    var aftertotalreceivedamount = 0
    var totalexcessamount = 0

    inoviceTotalAmount = $("#inoviceTotalAmount").val();
    totalexcessamount = $("#excessAmount").val();
         
    //totalreceivedamount = PaymentReceiveGriddata.reduce((s, a) => s + a.receivedAmount, 0);

    if (PaymentReceiveGriddata.length) {
        PaymentReceiveGriddata.forEach(c => {
            if (c.natureOfAmount == "CashRefund") {

                totalreceivedcashrefund += Number(c.receivedAmount);
            }
            if (c.natureOfAmount == "CreditAllowed") {

                totalreceivedcreditallow += Number(c.receivedAmount);
            }
            if (c.natureOfAmount != "CashRefund") {

                restoftotalreceivedcashrefund += Number(c.receivedAmount);
            }

        })
    }

    if (Number(totalreceivedcreditallow) <= 0) {



        aftertotalreceivedamount = restoftotalreceivedcashrefund - inoviceTotalAmount - (totalreceivedcashrefund + totalexcessamount);

    if (Number(aftertotalreceivedamount) <= Number(5000) && Number(aftertotalreceivedamount) > 0 && Number(inoviceTotalAmount) > 0) {

        let x = {
        aictBankName: "",
    receivedAmount: aftertotalreceivedamount,
    InsturmentNo: "",
    creditAllowed: 0,
    InoviceNo: "",
    KnockOffAmount: "",
    ...
    $("#excessAmount").val(0)
        ...
        PaymentNatureList = PaymentNatureList.filter(x => x.natureOfAmount != "CashRefund");

    $("#excessAmount").val(aftertotalreceivedamount);
    ...
    function GetcreatedInvoicesInfo() {

        if (type == "CFS") {

        $.get('/APICalls/GetInvoicesByIGMIndex?virno=' + igm + '&indexno=' + $("#indexdb option:selected").text() + '&type=' + type, function (resp) {
            ResultDataPreviousInvoice(resp);
        });
        }

    if (type == "CY") {

        $.get('/APICalls/GetInvoicesByIGMIndex?virno=' + igm + '&indexno=' + $("#containerIndexCYdb option:selected").text() + '&type=' + type, function (resp) {
    ...
    function ResultDataPreviousInvoice(resp) {
        var dataGrid = $("#createdInvoicesGrid").dxDataGrid({
        dataSource: resp,
    keyExpr: "invoiceId",
    showBorders: false,
    showBorders: false,
    ...
    {
        dataField: "afterSalesTax",
    dataType: "number",
    caption: "AfterSalesTax",
    format: "#,##0.##",
    editorOptions: {
        step: 0
                    }
    ...
    $("#islastPass").prop("checked", false);
    $("#ischeckAdvanceBill").prop("checked", false);
    LoadGrid('#tariffGrid', []);
    loadGridhold([]);
    ResultDataPreviousInvoice([]);
    $('#tbody').html('');
    ...
    $('#previousBillTotalCharges').val(0);
    $('#previousPaidBillToLineAmount').val(0);
    //$('#previousPaidGST').val(0);
    $('#afterSalesTax').val(0);
    $('#totalamount').val(0);
    $('#grandTotal').val(0);
    ...
    if (data.advanceDate != null) {
        ...
        $('#pass1').val(new Date(data.advanceDate.split("T")[0]).toISOString().substr(0, 10));
    ...
    $('#previousPaidBillToLineAmount').val(data.previousPaidBillToLineAmount);
    ...
    $('#totalArriveIndexes').val(data.totalArriveIndexes);
    ...
    $('#previousBillTotalCharges').val(data.previousBillTotalCharges);
    //$('#previousPaidGST').val(data.previousPaidGST);
    ...
    if (data.isSocCreated == false) {
        alert("Document Required For This Consigment are { " + data.documentCode + " } ");
                    }

    if (data.isLinkedWithFF == true) {
        alert("This Igm , index is not for ( AICT ) account payment Do you want to contineue !");
                    }
    ...
    var buttn = document.getElementById("socbuttonid");
    buttn.style.display = 'none';
    ...
    if (data) {
        data.forEach(c => {
            ...
            //second work start
            //var result = newarraydata.find(t => t.description == c.description && t.category == c.category && c.tariffType != "CYPerIndex");
            //if (result != null) {
            //    result.amount += c.amount;
            //}
            //else {
    ...
    $('#ffstorageShare').val(data.storageSharePercent);
    $('#aictPerBoxRate').val(data.aictPerBoxRate);
    ...
    if (indexCargoTyperes == "LCL") {

            if (Number($("#CBM").val()) < 1 ) {
                return alert("CBM must be greater or equals to 1 ");
            }
        }
    ...
    function calculateCounterCollection() {
        ...
        var buttn = document.getElementById("tabsdiv");
    buttn.style.display = 'block';
    ...
    function submitInvoice() {
        ...
        //setTimeout(function () {$("#btnSubmit").attr("disabled", false); }, 15000);
    ...
    var ledgerPartyid = $('#ledgerPartyid').val();
    ...
    if (islastPass == true &&   Number($('#ledgerPartyid').val()) ==  0  ) {
            //return showToast("please settle payment or select letpass you are selecting both", "error");
            return alert("your let pass party not avaible");
        }

    if (islastPass == true && $('#ledgerPartyid').val()  == "") {
            //return showToast("please settle payment or select letpass you are selecting both", "error");
            return alert("your let pass party not avaible");
        }
    ...
    // from SaveBill to GenerateInvoice
    $.post('/Invoice/GenerateInvoice?' + postData + "&islastPass=" + islastPass + "&partyId=" + ledgerPartyid , {invoiceItems: tariffdata, paymentReceivedList: PaymentReceiveGriddata} , function (data) {
    ...
    if (data.error == true) {
        ...
        alert(data.message); 
                        ...
                        if (data != "null") {
                            ...
                            if (data.isForLine == true) {

                                if (InvoiceNoBillToLine) {
        window.open('/import/reports/SalesTaxInvoice?BillNo=' + InvoiceNoBillToLine + '&BillType=' + "Bill To Line" + '&InvoiceCategory=' + "FF", "_blank");
                                }
    if (billNo) {
        window.open('/import/reports/SalesTaxInvoice?BillNo=' + billNo + '&BillType=' + "Normal" + '&InvoiceCategory=' + "FF", "_blank");
                                }
    ...
    if (AuctionInvoiveNUmber) {
        window.open('/import/reports/SalesTaxInvoiceAution?BillNo=' + AuctionInvoiveNUmber + '&BillType=' + "Auction" + '&InvoiceCategory=' + "FF", "_blank");
                                }
                            }
    else {
                                if (InvoiceNoBillToLine) {
        window.open('/import/reports/SalesTaxInvoice?BillNo=' + InvoiceNoBillToLine + '&BillType=' + "Bill To Line" + '&InvoiceCategory=' + "AICT", "_blank");
                                }

    if (billNo) {
        window.open('/import/reports/SalesTaxInvoice?BillNo=' + billNo + '&BillType=' + "Normal" + '&InvoiceCategory=' + "AICT", "_blank");
                                }
    if (DeliveryOrderNumber) {
        window.open('/import/reports/DeliveryOrder?DeliveryOrderNumber=' + DeliveryOrderNumber, "_blank");
                                }
    if (AuctionInvoiveNUmber) {
        window.open('/import/reports/SalesTaxInvoiceAution?BillNo=' + AuctionInvoiveNUmber + '&BillType=' + "Auction" + '&InvoiceCategory=' + "AICT", "_blank");
                                }
                            }
    ...
    function formfiled() {
        ...
        //if (PermissionInputs.find(x => x.fieldName == "SHOW IMPORT BILL DETAIL" && x.isChecked == true)) {
        //    var panel = document.getElementsByClassName('panelforshowhide')[0];             
        ...
        document.getElementById('ischeckAdvanceBill').onchange = function () {
            ...
            document.getElementById('pass1').readOnly = !this.checked;
        }


    }

    function showhidepagedetail() {
         
        if (document.getElementById("showdetailbar").style.display == "block") {
        document.getElementById("showdetailbar").style.display = "none";
        }
    else {

        document.getElementById("showdetailbar").style.display = "block";
        }
    $('#agents').select2().trigger('change.select2');
    }
    ...
    if (deliveryStatus == "Delivered") {
        ...
        alert("can't create waiver due to cargo is delivered");
        }
    else {
            if (type == "CFS") {

                var containerid;
    var remarks = $('#WaiveRemarks').val();
    ...
    containerid = $("#indexdb option:selected").val();



    if (remarks && type && containerid) {
    ...
    if (tariffdata.length) {
        ...
        //$.post('/Invoice/AddWaiVerDetail?type=' + type + '&remarks=' + remarks + '&containerid=' + containerid + '&storageAmount=' + storageAmount
        //    + '&sealChargr=' + sealChargr + '&portChargAmount=' + portChargAmount + '&otherChargAmount=' + otherChargAmount + '&vehicleChargs=' + vehicleChargs
        //    , { tariffdata: tariffdata }, function (data) {
            ...
            , { tariffdata: tariffdata }, function (data) {
                ...
                if (data.error == true) {
                    ...
                    if (data.waiverno) {
                        ...
                        window.open('/Import/Reports/RequestForWaiver?waiverno=' + data.waiverno + '&type=' + type + '&containerid=' + containerid, "_blank");
                    }
        ...
        //showToast(" please add tariffs", "error")
        alert(" please add tariffs");
        ...
        //showToast("please select type igm and index , remarks", "error")
        alert("please select type igm and index , remarks");
                }
    ...
    if (remarks && type && igm && indexno) {

                    var tariffdata = $("#tariffGrid").dxDataGrid("option", "dataSource");

    if (tariffdata.length) {
        //$.post('/Invoice/AddWaiVerDetailCY?type=' + type + '&remarks=' + remarks + '&igm=' + igm + '&indexno=' + indexno + '&storageAmount=' + storageAmount
        //    + '&sealChargr=' + sealChargr + '&portChargAmount=' + portChargAmount + '&otherChargAmount=' + otherChargAmount + '&vehicleChargs=' + vehicleChargs
        //    + '&cyStoragSizeAmount=' + cyStoragSizeAmount, { tariffdata: tariffdata }, function (data) {
            ...
            + '&cyStoragSizeAmount=' + cyStoragSizeAmount, { tariffdata: tariffdata }, function (data) {
                ...
                if (data.error == true) {
                    ...
                    if (data.waiverno) {
                        ...
                        window.open('/Import/Reports/RequestForWaiver?waiverno=' + data.waiverno + '&type=' + type + '&containerid=' + data.containerId, "_blank");
                    }
    ...
    //#region for new calculation format

    //var perviouspaidamount = $('#previousBillAmount').val();
    //var totalInvoiceAmountExWaiver = $('#totalInvoiceAmountExWaiver').val();


    var totalChargesres = Number($('#totalCharges').val());
    var previousPaidBillToLineAmountres = Number($('#previousPaidBillToLineAmount').val());
    var previousWaiVerAmountres = Number($('#previousWaiVerAmount').val());
    var previousBillTotalChargesres = Number($('#previousBillTotalCharges').val());
    var waiverDiscountAmountres = Number($('#waiverDiscountAmount').val());
    var salesTax = $('#salesTax').val();
        ...
        $.post('/Invoice/CalculateBillToLineCFS?storageAmount=' + storageAmount + '&sealChargr=' + sealChargr + '&otherChargAmount=' + otherChargAmount
            + '&vehicleChargs=' + vehicleChargs + '&igm=' + igm + '&indexno=' + indexno + '&type=' + type, { tariffdata: tariffdata }, function (data) {
                ...
                if (data > 0) {
                    ...
                    //commint due to sales tax working worng on sub bills when sales tax change from 13 to 15
                    //var previousBillAmountandbtlwithgst = Number(perviouspaidamount) + Number($('#billTOLINEIncGSTD').val());

                    //var previousBillAmountandbtlwithgst =   Number($('#billTOLINEIncGSTD').val());

                    //commit due to auciton work change add new line in below var aftertotalInvoiceAmountExWaiver = ((Number(totalInvoiceAmountExWaiver) + Number($('#auctionGrandTotal').val())) - Number(previousBillAmountandbtlwithgst));
                    //var aftertotalInvoiceAmountExWaiver = (Number(totalInvoiceAmountExWaiver)   - Number(previousBillAmountandbtlwithgst));

                    var currentbtlamount = Number($('#billToLineAmount').val());
                    var totalchargesafterwaiverandbilltoline = (totalChargesres - previousPaidBillToLineAmountres - previousWaiVerAmountres - previousBillTotalChargesres - waiverDiscountAmountres - currentbtlamount);
                    $('#totalChargesAfterWaiver').val(Number(totalchargesafterwaiverandbilltoline));

                    var invoicegstamount = Math.ceil(Number(totalchargesafterwaiverandbilltoline) * (Number(salesTax) / 100));
                    $('#gst').val(invoicegstamount);


                    $('#totalInvoiceAmountExWaiver').val(Math.ceil(Number($('#gst').val()) + Number(totalchargesafterwaiverandbilltoline)));

                    var aftertotalInvoiceAmountExWaiver = $('#totalInvoiceAmountExWaiver').val();


                    $("#invoiceAmount").val(Number(aftertotalInvoiceAmountExWaiver) + Number(waiverDiscountAmountres));
                    ...
                    if (counterCollectionres < 0) {
                        ...
                        totalbalanceInoviceAmount = (((aftertotalInvoiceAmountExWaiver - totalpaymentreceivedamount)) + (Number($("#excessAmount").val())));
                        if (totalbalanceInoviceAmount < 0) {
                            totalbalanceInoviceAmount = 0;
                        }
                        $("#balanceInoviceAmount").val(totalbalanceInoviceAmount);
                    ...
                    //$('#counterCollection').val(aftertotalInvoiceAmountExWaiver - Number(perviouspaidamount) );
    ...
    $.post('/Import/Reports/ScheduleofCharges?' + postData, {invoiceItems: tariffdata }, function (data) {
        ...
        console.log("data  ")
                            if (data.invoiceNo) {
        console.log("datacasd  ")
                                window.open('/import/reports/SOCReportView?SOCId=' + data.invoiceNo, "_blank");
                            }
                        }
                    }
    ...
    function tabevent(value) {
        ...
        if (value == "CounterPayment") {
            if ($('#islastPass').is(":checked") == true) {
        document.getElementById("anf-tab").disabled = true;
    alert("Once the letpass is check, it is not possible to make a counter payment, ");
            }
    else {
        document.getElementById("anf-tab").disabled = false
    }
            
        }
    else if (value == "LetPassPayment"  ) {

            if ($("#PaymentReceiveGrid").dxDataGrid("option", "dataSource").length) {
        document.getElementById("paperready-tab").disabled = true;
    alert("Once the counter payment is feeded, it is not possible to make a Letpass, ");
            }
    else {
        document.getElementById("paperready-tab").disabled = false;
            }
